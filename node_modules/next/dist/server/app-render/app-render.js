    ...
    !isBypassingCachesInDev(renderOpts, initialRequestStore)) {
        ...
        if (resolveValidation) {
            ...
            _consoleasyncstorageexternal.consoleAsyncStorage.run({
                dim: true
            }, spawnStaticShellValidationInDev, resolveValidation, staticChunks, runtimeChunks, dynamicChunks, staticInterruptReason, runtimeInterruptReason, staticStageEndTime, runtimeStageEndTime, ctx, clientReferenceManifest, finalRequestStore, devFallbackParams, validationDebugChannelClient);
        }
...
function App({ reactServerStream, reactDebugStream, debugEndTime, preinitScripts, clientReferenceManifest, ServerInsertedHTMLProvider, nonce, images }) {
    preinitScripts();
    const response = _react.use((0, _useflightresponse.getFlightStream)(reactServerStream, reactDebugStream, debugEndTime, clientReferenceManifest, nonce));
...
// consistent for now.
function ErrorApp({ reactServerStream, preinitScripts, clientReferenceManifest, ServerInsertedHTMLProvider, nonce, images }) {
    /* eslint-disable @next/internal/no-ambiguous-jsx -- React Client */ preinitScripts();
    const response = _react.use((0, _useflightresponse.getFlightStream)(reactServerStream, undefined, undefined, clientReferenceManifest, nonce));
    const initialState = (0, _createinitialrouterstate.createInitialRouterState)({
        // This is not used during hydration, so we don't have to pass a
        // real timestamp.
        navigatedAt: -1,
...
async function renderWithRestartOnCacheMissInDev(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    const maybeInitialServerStream = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, ()=>(0, _apprenderrenderutils.pipelineInSequentialTasks)(()=>{
            ...
            accumulateStreamChunks(accumulatingStream, staticChunks, runtimeChunks, dynamicChunks, initialStageController, initialDataController.signal);
            ...
            // Dynamic stage
            if (stream === null || initialStageController.currentStage === _stagedrendering.RenderStage.Abandoned) {
    ...
    const finalServerStream = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, ()=>(0, _apprenderrenderutils.pipelineInSequentialTasks)(()=>{
            ...
            const stream = ComponentMod.renderToReadableStream(finalRscPayload, clientReferenceManifest.clientModules, {
                onError,
                environmentName,
                filterStackFrame,
                debugChannel: debugChannel == null ? void 0 : debugChannel.serverSide
            ...
            const [continuationStream, accumulatingStream] = stream.tee();
            accumulateStreamChunks(accumulatingStream, staticChunks, runtimeChunks, dynamicChunks, finalStageController, null);
            return continuationStream;
        }, (stream)=>{
            // Runtime stage
...
async function accumulateStreamChunks(stream, staticTarget, runtimeTarget, dynamicTarget, stageController, signal) {
 ...
 */ async function spawnStaticShellValidationInDev(resolveValidation, staticServerChunks, runtimeServerChunks, dynamicServerChunks, staticInterruptReason, runtimeInterruptReason, staticStageEndTime, runtimeStageEndTime, ctx, clientReferenceManifest, requestStore, fallbackRouteParams, debugChannelClient) {
    ...
    if (invalidDynamicUsageError) {
        resolveValidation(createElement(ReportValidation, {
            messages: [
    ...
    await warmupModuleCacheForRuntimeValidationInDev(runtimeServerChunks, dynamicServerChunks, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest);
    let debugChunks = null;
    if (debugChannelClient) {
        debugChunks = [];
        debugChannelClient.on('data', (c)=>debugChunks.push(c));
    }
    const runtimeResult = await validateStagedShell(runtimeServerChunks, dynamicServerChunks, debugChunks, runtimeStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest, hmrRefreshHash, _dynamicrendering.trackDynamicHoleInRuntimeShell);
    ...
    const staticResult = await validateStagedShell(staticServerChunks, dynamicServerChunks, debugChunks, staticStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest, hmrRefreshHash, _dynamicrendering.trackDynamicHoleInStaticShell);
    // We always resolve with whatever results we got. It might be empty in which
    // case there will be nothing to report once
    resolveValidation(createElement(ReportValidation, {
        messages: staticResult
    }));
    return;
}
async function warmupModuleCacheForRuntimeValidationInDev(runtimeServerChunks, allServerChunks, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest) {
...
async function validateStagedShell(stageChunks, allServerChunks, debugChunks, debugEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest, hmrRefreshHash, trackDynamicHole) {
    const { implicitTags, nonce, workStore } = ctx;
...
function ReportValidation({ messages }) {
