...
import { startPPRNavigation, startPPRRefresh, listenForDynamicRequest } from '../router-reducer/ppr-navigations';
...
export function refresh(currentUrl, currentFlightRouterState, currentNextUrl, currentRenderedSearch, currentCanonicalUrl) {
    ...
    const task = startPPRRefresh(now, currentFlightRouterState, currentNextUrl, accumulation);
    if (task !== null) {
        if (task.dynamicRequestTree !== null) {
            ...
            const includeNextUrl = hasInterceptionRouteInCurrentTree(currentFlightRouterState);
            listenForDynamicRequest(currentUrl, includeNextUrl ? currentNextUrl : null, task, task.dynamicRequestTree, null, accumulation);
        }
...
function navigateUsingPrefetchedRouteTree(now, url, currentUrl, nextUrl, isSamePageNavigation, currentCacheNode, currentFlightRouterState, prefetchFlightRouterState, prefetchSeedData, prefetchHead, isPrefetchHeadPartial, canonicalUrl, renderedSearch, shouldScroll, hash) {
    ...
    const task = startPPRNavigation(now, currentUrl, currentCacheNode, currentFlightRouterState, prefetchFlightRouterState, prefetchSeedData, prefetchHead, isPrefetchHeadPartial, isSamePageNavigation, accumulation);
    if (task !== null) {
        if (task.dynamicRequestTree !== null) {
...
async function navigateDynamicallyWithNoPrefetch(now, url, currentUrl, nextUrl, isSamePageNavigation, currentCacheNode, currentFlightRouterState, shouldScroll, hash, collectedDebugInfo) {
    ...
    if (task !== null) {
        ...
        return navigationTaskToResult(task, currentCacheNode, createHrefFromUrl(canonicalUrl), renderedSearch, accumulation.scrollableSegments, shouldScroll, hash);
    }
    // The server sent back an empty tree patch. There's nothing to update, except
    // possibly the URL.
    return {
