...
function App({ reactServerStream, reactDebugStream, debugEndTime, preinitScripts, clientReferenceManifest, ServerInsertedHTMLProvider, nonce, images }) {
    preinitScripts();
    const response = ReactClient.use(getFlightStream(reactServerStream, reactDebugStream, debugEndTime, clientReferenceManifest, nonce));
...
function ErrorApp({ reactServerStream, preinitScripts, clientReferenceManifest, ServerInsertedHTMLProvider, nonce, images }) {
    ...
    const response = ReactClient.use(getFlightStream(reactServerStream, undefined, undefined, clientReferenceManifest, nonce));
    const initialState = createInitialRouterState({
        // This is not used during hydration, so we don't have to pass a
        // real timestamp.
        navigatedAt: -1,
...
async function renderWithRestartOnCacheMissInDev(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    const maybeInitialServerStream = await workUnitAsyncStorage.run(requestStore, ()=>pipelineInSequentialTasks(()=>{
            ...
            // Dynamic stage
            if (stream === null || initialStageController.currentStage === RenderStage.Abandoned) {
 ...
 */ async function spawnStaticShellValidationInDev(resolveValidation, staticServerChunks, runtimeServerChunks, dynamicServerChunks, staticInterruptReason, runtimeInterruptReason, staticStageEndTime, runtimeStageEndTime, ctx, clientReferenceManifest, requestStore, fallbackRouteParams, debugChannelClient) {
    ...
    const runtimeResult = await validateStagedShell(runtimeServerChunks, dynamicServerChunks, debugChunks, runtimeStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest, hmrRefreshHash, trackDynamicHoleInRuntimeShell);
    ...
    const staticResult = await validateStagedShell(staticServerChunks, dynamicServerChunks, debugChunks, staticStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, clientReferenceManifest, hmrRefreshHash, trackDynamicHoleInStaticShell);
